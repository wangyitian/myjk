...
class YuanchenghuizhenController: UIViewController ,UITableViewDataSource,UITableViewDelegate{
    ...
    @IBAction func apply(){
        ...
        let ctl = UIStoryboard(name: "Mine", bundle: nil).instantiateViewControllerWithIdentifier("ModifyInfoController") as! ModifyInfoController
        ...
        ctl.navigationItem.rightBarButtonItem = UIBarButtonItem(customView: skipView)
        ctl.toPay = true
        ...
        if indexPath.section == 0{
            ...
            if let bannerInfo = NSUserDefaults.standardUserDefaults().objectForKey(kBannerInfoKey) as? [AnyObject]{
                if bannerInfo.count > 3{
                    if let banner = bannerInfo[0] as? [String:String]{
    ...
    func fetchData(){
        SharedNetWorkManager.GET(kYuanChengUrlString, parameters: nil, success: { (task, result) -> Void in

...
class FumeiyiliaoController: UIViewController  ,UITableViewDataSource,UITableViewDelegate{
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        navigationItem.leftBarButtonItem = GetLeftBarButtonItem(self, action: #selector(FumeiyiliaoController.back))
        ...
        rBtn.addTarget(self, action: #selector(FumeiyiliaoController.appply), forControlEvents: UIControlEvents.TouchUpInside)
        ...
        if indexPath.section == 0{
            ...
            if let bannerInfo = NSUserDefaults.standardUserDefaults().objectForKey(kBannerInfoKey) as? [AnyObject]{
                if bannerInfo.count > 3{
                    if let banner = bannerInfo[1] as? [String:String]{
